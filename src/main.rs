...
fn create_schedule<'a>(raw_events: &[Event<'a>], room_count: usize, time_slots: usize) -> Box<[Box<[Option<&'a str>]>]> {
    
    let mut schedule: Box<[Box<[Option<&str>]>]> = vec![vec![None; room_count].into_boxed_slice(); time_slots].into_boxed_slice();
    
    let mut event_map: HashMap<&str, (&Event, Option<Placement>)> = raw_events.iter().map(|event| (event.name, (event, None))).collect();
    ...
    loop {
        ...
        for constraint in &curr_event.as_ref().unwrap().constraints { //curr_event.take().unwrap().constraints.iter() {
                        ...
                        if let (event, Some(placement)) = event_map.get(item).unwrap() {
                            ...
                            match constraint {
                                Constraint::Sequential(items) => {
                                    ...
                                    let placed_pos = items.iter().position(|e| *e == event.name).unwrap();
                                    
                                    let placing_pos = items.iter().position(|e| *e == curr_event.as_ref().unwrap().name).unwrap();

                                    dbg!(placing_pos, placed_pos);

                                    let seq_time = (placement.time.time_slot + placing_pos).checked_sub(placed_pos);
                                    ...
                                    if time.is_none() && room.is_none() {
                                        time = Some(Time{time_slot: seq_time.unwrap()});
        ...
        dbg!("loop");
        let placement = loop {// while time.is_none() && room.is_none() {
        
            
            let try_time = time.clone().unwrap_or(Time { time_slot: time_index} );
            dbg!(&time);
            let try_room = room.clone().unwrap_or(Room { room_num: room_index});
        ...
        schedule[placement.time.time_slot][placement.room.room_num] = Some(curr_event.unwrap().name);
